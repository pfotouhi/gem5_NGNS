...
class Torus_XY_4C(SimpleTopology):
    description='Torus_XY_4C'
            ...
            for col in xrange(num_columns):
                if (col + 1 < num_columns):
                    ...
                    int_links.append(IntLink(link_id=link_count,
                                             ...
                                             weight=1))
                    link_count += 1
                # Torus links: connect node in last column (east_out) to node in first column (west_in) 
                elif (col == num_columns - 1):
                    east_out = col + (row * num_columns)
                    west_in = 0 + (row * num_columns)
                    int_links.append(IntLink(link_id=link_count,
                                             src_node=routers[east_out],
                                             dst_node=routers[west_in],
                                             src_outport="East",
	...
	# West output to East input links (weight = 1)
        for row in xrange(num_rows):
            for col in xrange(num_columns):
                if (col == 0): # torus links: connect node in first column (west_out) to node in last column (east_in)
                    east_in = num_columns - 1 + (row * num_columns)
                    west_out = 0 + (row * num_columns)
                    int_links.append(IntLink(link_id=link_count,
	...
	# North output to South input links (weight = 2)
        for col in xrange(num_columns):
            for row in xrange(num_rows):
                if (row == 0): # Torus links: connect node in first row (north_out) to node in last row (south_in)
                    north_out = col
                    south_in  = col + ((num_rows - 1) * num_columns)
                    int_links.append(IntLink(link_id=link_count,
            ...
            for row in xrange(num_rows):
                if (row + 1 < num_rows):
                    ...
                    int_links.append(IntLink(link_id=link_count,
                                             ...
                                             weight=2))
                    link_count += 1
                elif (row == num_rows - 1): # torus links: connect node in last row (south_out) to node in first row (north_in)
                    north_in = col
                    south_out = col + (num_rows - 1) * num_columns
                    int_links.append(IntLink(link_id=link_count,
                                             src_node=routers[south_out],
                                             dst_node=routers[north_in],
                                             src_outport="South",
